# dubbing_factory.py — V14.0 (Dedicated Dubbing Service)
import modal
from fastapi import FastAPI, UploadFile, File, Form
from fastapi.responses import JSONResponse
import os, tempfile, subprocess, shutil, re, json, requests, uuid, gc, asyncio, logging
...
image = (
    ...
    .pip_install(
        ...
        "deep-translator", "pydub", "google-cloud-storage", "python-multipart"
    )
...
class DubbingService:
    ...
    def fastapi_app(self):
        web_app = FastAPI()
        @web_app.post("/upload")
        async def upload(media_file: UploadFile = File(...), lang: str = Form("en"), voice_id: str = Form("")):
            ...
            for seg in segments:
                ...
                out_seg = os.path.join(temp_dir, f"seg.wav")
                self.xtts.tts_to_file(text=txt, file_path=out_seg, language=lang)
