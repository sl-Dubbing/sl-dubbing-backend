...
def token_required(f):
    ...
    def decorated(*args, **kwargs):
        ...
        if not supa_user: return jsonify({'error': 'Invalid token'}), 401

        user = User.query.filter_by(email=supa_user.get('email')).first()
        if not user:
            user = User(email=supa_user.get('email'), name=supa_user.get('email').split('@')[0], supabase_id=supa_user.get('id'), credits=10)
            db.session.add(user)
            db.session.commit()
...
def get_history(user):
    ...
    history = [{
        ...
        "filename": j.input_key.split('/')[-1] if j.input_key else "معالجة",
...
def start_dubbing(user):
    ...
    if not file_key: return jsonify({'error': 'file_key required'}), 400
    
    media_url = s3.generate_presigned_url('get_object', Params={'Bucket': R2_BUCKET, 'Key': file_key}, ExpiresIn=7200)
    
    job = DubbingJob(user_id=user.id, kind='dub', lang=data.get('lang', 'en'), voice_id=data.get('voice_id', 'source'), engine=data.get('engine', ''), status='queued', input_key=file_key, created_at=datetime.utcnow())
    db.session.add(job)
    db.session.commit()
