# server.py — النسخة V7.7 (Production ready: unified MAX_UPLOAD_BYTES via env, session with retry, health check, robust cleanup)
import os
import uuid
import json
import base64
...
# تقليل الحد الأقصى للملفات: موحّد عبر env
...
# عدد العمال الافتراضي؛ قابل للتعديل عبر env
...
# Configure a requests.Session with retries for background uploads
...
# Optional quick health check for MODAL_URL at startup
if MODAL_URL:
    try:
    ...
    logging.warning("MODAL_URL not configured; background tasks will fail until set.")
...
# ── 3. محرك المعالجة الخلفية (محدّث: رفع multipart إلى factory مع session و retry) ──
def run_background_task(job_id, endpoint, payload, cost, user_id):
    """
    Background task: if payload contains '_file_path' we upload multipart,
    otherwise fallback to JSON payload (legacy).
    Uses a shared requests.Session with retry/backoff.
    """
    with app.app_context():
        job = DubbingJob.query.get(job_id)
        user = User.query.get(user_id)
        if not job or not user:
            logging.error("Background task: job or user not found (job_id=%s, user_id=%s)", job_id, user_id)
            return

        file_path = None
        try:
            if not MODAL_URL:
                raise RuntimeError("MODAL_URL not configured")

            # prefer explicit upload endpoint
            target = f"{MODAL_URL.rstrip('/')}/{endpoint.lstrip('/')}" if endpoint else f"{MODAL_URL.rstrip('/')}/upload"

            # If payload contains a local file path, send multipart
            file_path = payload.pop("_file_path", None)
            if file_path and os.path.exists(file_path):
                with open(file_path, "rb") as fh:
                    files = {'media_file': (os.path.basename(file_path), fh)}
                    data = {k: v for k, v in payload.items() if v is not None}
                    # use session with retry and reasonable timeout
                    res = _http_session.post(target, data=data, files=files, timeout=int(os.environ.get("FACTORY_TIMEOUT", "120")))
            else:
                # legacy: send JSON (file_b64)
            ...
            try:
                ...
                raise RuntimeError("Factory returned non-JSON response")
            ...
            if data.get("success"):
                ...
                raise RuntimeError(data.get("error", "Unknown factory error"))
            ...
            try:
                ...
                logging.exception("Failed to refund credits for user %s", user_id)
        finally:
            # commit DB changes safely
            try:
                db.session.commit()
            except Exception:
                logging.exception("Failed to commit DB changes for job %s", job_id)
                try:
                    db.session.rollback()
                except Exception:
                    pass

            # cleanup local file and parent tempdir if present
            try:
                if file_path and os.path.exists(file_path):
                    try:
                        os.remove(file_path)
                    except Exception:
                        logging.exception("Failed to remove file %s", file_path)
                    parent = os.path.dirname(file_path)
                    try:
                        if os.path.isdir(parent):
                            shutil.rmtree(parent, ignore_errors=True)
                    except Exception:
                        logging.exception("Failed to remove parent tempdir %s", parent)
            except Exception:
                logging.exception("Cleanup after background task failed for %s", file_path)


# ── Helpers for upload processing ──
...
def _create_job_and_submit(file_path: str, lang: str, voice_id: str, sample_b64: str, user, cost: int):
    """
    Create DB job, deduct credits, submit background task.
    Returns job_id on success or raises Exception.
    """
    job_id = str(uuid.uuid4())
    job = DubbingJob(id=job_id, user_id=user.id, status='processing',
                     language=lang, voice_mode=voice_id or 'xtts')
    # deduct credits
    user.credits = (user.credits or 0) - cost
    db.session.add(job)
    db.session.commit()
...
def _start_dub_job_from_fileobj(file_obj, filename: str, user, form, cost: int = 100):
    """
    Writes file_obj to temp disk, validates size, creates job and submits it.
    Returns (job_id, error_tuple) where error_tuple is None on success or (message, status_code).
    """
    if not _allowed_file(filename):
        return None, ("Unsupported file type", 415)

    temp_dir = tempfile.mkdtemp()
    input_path = os.path.join(temp_dir, "input.bin")
    try:
        # write in chunks to disk to avoid memory spikes
        try:
            file_obj.seek(0)
        except Exception:
            # some streams are not seekable; ignore
            pass
        ...
        if size > MAX_UPLOAD_BYTES:
            # cleanup tempdir on size exceed
            try:
                shutil.rmtree(temp_dir, ignore_errors=True)
            except Exception:
                pass
            return None, ("File is too large. Maximum size is {}MB.".format(MAX_UPLOAD_BYTES // (1024*1024)), 413)

        lang = form.get('lang', 'en')
        voice_id = form.get('voice_id', '')
        sample_b64 = form.get('sample_b64', '')

        # create job and submit (pass file path to background task)
        job_id = _create_job_and_submit(input_path, lang, voice_id, sample_b64, user, cost)
        # do NOT delete temp_dir here — background task will remove file after upload
        return job_id, None
    except Exception as e:
...
def dub():
    ...
    if content_length and content_length > MAX_UPLOAD_BYTES:
        return jsonify({"error": "File is too large. Maximum size is {}MB.".format(MAX_UPLOAD_BYTES // (1024*1024))}), 413
...
# New endpoint: /upload (multipart) — same processing logic as /api/dub
@app.route('/upload', methods=['POST'])
@require_auth
def upload():
    """
    Accepts multipart/form-data with field 'media_file' and optional form fields:
    - lang
    - voice_id
    - sample_b64
    Delegates to the same processing logic as /api/dub.
    """
    user = request.user
    cost = 100
