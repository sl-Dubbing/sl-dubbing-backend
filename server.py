# server.py — V3.2 File Manager Edition
import os
import uuid
import logging
...
def get_history(user):
    # سياسة الـ 15 يوماً: جلب الملفات التي لم يتجاوز عمرها 15 يوماً فقط
...
# (بقية مسارات الرفع والدبلجة كما هي في النسخة السابقة...)
...
def start_dubbing(user):
    ...
    job = DubbingJob(user_id=user.id, kind='dub', lang=data.get('lang', 'en'), status='queued', input_key=file_key, created_at=datetime.utcnow())
    db.session.add(job)
    db.session.commit()
